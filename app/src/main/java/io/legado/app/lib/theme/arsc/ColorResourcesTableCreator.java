...
/**
 * Writes an ARSC resource table that overrides the given color resources at runtime.
 *
 * <p>Ported from material-components-android 1.13.0
 * {@code com.google.android.material.color.ColorResourcesTableCreator} with these adaptations:
 *
 * <ul>
 *   <li>Pure JVM: no {@code Context}/{@code Resources}/{@code androidx.annotation} dependency.
 *       Entry names are resolved through the injected {@link IntFunction} (the caller passes
 *       {@code resources::getResourceEntryName}); unit tests inject deterministic fake names.
 *   <li>The runtime color-type check is removed: callers pass compile-time {@code R.color}
 *       identifiers, so the resource type is guaranteed by construction (precondition).
 *   <li>{@code typeIdColor} is threaded through constructors instead of a mutable static.
 * </ul>
 *
 * <p>This class consists of definitions of resource data structures and helps create a Color
...
public final class ColorResourcesTableCreator {
  ...
  /**
   * Creates the bytes of an ARSC table overriding the given colors.
   *
   * @param packageName name of the application package (package id 0x7F in the table)
   * @param colorMapping color resource id -> ARGB value; ids must be app-package (0x7F) color
   *     resources with a non-zero type id
   * @param entryNameResolver resolves a resource id to its entry name
   */
  public static byte[] create(
      String packageName, Map<Integer, Integer> colorMapping, IntFunction<String> entryNameResolver)
      throws IOException {
    if (colorMapping.isEmpty()) {
      throw new IllegalArgumentException("No color resources provided.");
    }
    PackageInfo applicationPackageInfo = new PackageInfo(APPLICATION_PACKAGE_ID, packageName);

    List<ColorResource> colorResources = new ArrayList<>();
    for (Map.Entry<Integer, Integer> entry : colorMapping.entrySet()) {
      ColorResource colorResource =
          new ColorResource(entry.getKey(), entryNameResolver.apply(entry.getKey()),
              entry.getValue());
      if (colorResource.packageId != APPLICATION_PACKAGE_ID) {
        throw new IllegalArgumentException(
            "Not supported with unknown package id: " + colorResource.packageId);
      }
      if (colorResource.typeId == 0) {
        throw new IllegalArgumentException("Not a color resource: id=" + entry.getKey());
      }
      colorResources.add(colorResource);
    }
    byte typeIdColor = colorResources.get(0).typeId;
    ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
    new ResTable(applicationPackageInfo, colorResources, typeIdColor).writeTo(outputStream);
  ...
  private static class ResTable {
    ...
    ResTable(PackageInfo packageInfo, List<ColorResource> colorResources, byte typeIdColor) {
      packageCount = 1;
      stringPool = new StringPoolChunk();
      Collections.sort(colorResources, COLOR_RESOURCE_COMPARATOR);
      packageChunks.add(new PackageChunk(packageInfo, colorResources, typeIdColor));
  ...
  private static class StringPoolChunk {
    ...
    StringPoolChunk(boolean utf8, String... rawStrings) {
      ...
      for (String string : rawStrings) {
        ProcessedString processedString = processString(string);
        stringIndex.add(stringOffset);
        stringOffset += processedString.bytes.length;
        strings.add(processedString.bytes);
        styledSpans.add(processedString.styledSpans);
      }
    ...
    private ProcessedString processString(String rawString) {
      // Ignore styled spans, won't be used in our scenario.
      return new ProcessedString(
          utf8Encode ? stringToByteArrayUtf8(rawString) : stringToByteArray(rawString),
          Collections.<StringStyledSpan>emptyList());
    }
  }

  private static final class ProcessedString {
    final byte[] bytes;
    final List<StringStyledSpan> styledSpans;

    ProcessedString(byte[] bytes, List<StringStyledSpan> styledSpans) {
      this.bytes = bytes;
  ...
  private static class PackageChunk {
    ...
    PackageChunk(PackageInfo packageInfo, List<ColorResource> colorResources, byte typeIdColor) {
      this.packageInfo = packageInfo;

      typeStrings = new StringPoolChunk(false, generateTypeStrings(colorResources, typeIdColor));
      keyStrings = new StringPoolChunk(true, generateKeyStrings(colorResources));
      typeSpecChunk = new TypeSpecChunk(colorResources, typeIdColor);

      header = new ResChunkHeader(HEADER_TYPE_PACKAGE, HEADER_SIZE, getChunkSize());
    }

    private String[] generateTypeStrings(List<ColorResource> colorResources, byte typeIdColor) {
      if (!colorResources.isEmpty()) {
        String[] types = new String[typeIdColor & 0xFF];

        // Placeholder String type, since only XML color resources will be replaced at runtime.
        for (int i = 0; i < (typeIdColor & 0xFF) - 1; i++) {
          types[i] = "?" + (i + 1);
        }

        types[(typeIdColor & 0xFF) - 1] = "color";
  ...
  private static class TypeSpecChunk {
    ...
    TypeSpecChunk(List<ColorResource> colorResources, byte typeIdColor) {
      ...
      typeChunk = new TypeChunk(colorResources, validEntryIds, entryCount, typeIdColor);
    }

    void writeTo(ByteArrayOutputStream outputStream) throws IOException {
      header.writeTo(outputStream);
      outputStream.write(new byte[] {typeChunk.typeIdColor, 0x00, 0x00, 0x00});
  ...
  private static class TypeChunk {
    ...
    private final byte typeIdColor;
    private final ResChunkHeader header;
    private final int entryCount;
